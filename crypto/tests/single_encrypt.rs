...
use crate::common::{
    clean_tests_path, generate_random_plaintext_file, generate_seeded_key, init_test_path,
};
...
async fn encrypt_decrypt_with_key(key: &[u8; 32]) {
    ...
    let decryptor = FileDecryptor::try_new(encrypted_path, plaintext_recovered_path, key).unwrap();
...
async fn small_file_seeded_key() {
    ...
    for length in tests_file_size {
        ...
        encrypt_decrypt_with_key(&generate_seeded_key()).await;
    }
